...
<h2 class="text-lg font-semibold text-gray-700 capitalize">Roles</h2>
<div class="mb-4 text-right">
    
        <a href="roles/create" class="inline-block px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700">Nuevo registro</a>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach($roles as $key => $rol)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 border">{{ $rol->clave }}</td>
                        <td class="px-4 py-2 border">{{ $rol->nombre }}</td>
                        <td class="px-4 py-2 border">{{ $rol->descripcion }}</td>
                        <td class="px-4 py-2 border">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium 
                            {{ $rol->estatus == 'Activo' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $rol->estatus }}
                            </span>
                        </td>
                        <td class="px-4 py-2 border">
                            <div class="flex items-center space-x-2">
                                <!-- Ver -->
                                <a href="roles/{{ $rol->id_rol }}" class="inline-flex items-center justify-center w-8 h-8 text-green-600 border border-green-600 rounded hover:bg-green-600 hover:text-white">
                                ...
                                <!-- Editar -->
                                <a href="roles/{{ $rol->id_rol }}/edit" class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 border border-yellow-600 rounded hover:bg-yellow-600 hover:text-white">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                                ...
                                <!-- Eliminar -->
                                <form action="roles/{{ $rol->id_rol }}" method="POST" class="inline">
                                    {!! csrf_field() !!}
                                    @method("delete")
