        ...
        <div class="flex justify-end mt-2 mb-4">
            <nav class="text-sm text-gray-600">
                <div class="flex items-center space-x-4">
                    ...
                    <!-- Usuarios -->
                    <li class="flex items-center">
                        <a href="{{ route('usuarios.index') }}" title="Volver a la página de usuarios"
                            class="text-gray-800 hover:text-gray-800">
                            Usuarios
                        </a>
                    </li>
            ...
            <div class="p-6 mx-auto bg-white rounded-lg shadow-lg max-w-7xl">
                    ...
                    @if ($usuario->foto != 'shadow.png')
                        <div>
                            <img src="{{ url('img/usuarios/' . $usuario->foto) }}" alt="Foto de Usuario"
                                class="object-cover w-16 h-16 border-4 border-indigo-500 rounded-full">
                            <a href="{{ route('usuario', $usuario->id_usuario) }}" class="text-sm text-gray-500">Eliminar
                                foto</a>
                        </div>
                    @else
                        <div>
                ...
                <form action="{{ route('usuarios.update', $usuario->id_usuario) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 gap-6 pt-3 sm:grid-cols-2 md:grid-cols-3">
                        ...
                        <div>
                            <label for="num_empleado" class="block text-base font-medium text-gray-700">Nùmero de Seguro
                                Social (NSS)</label>
                            <input type="text" name="num_empleado" id="num_empleado"
                                value="{{ old('num_empleado', $usuario->num_empleado) }}" placeholder="AA-BB-CC-DDDD-E"
                                ...
                                value="{{ old('num_empleado') }}" title="Actualiza el número de seguro social">
                            @error('num_empleado')
                        ...
                        <div>
                            ...
                            <input type="text" name="nombre" id="nombre"
                                value="{{ old('nombre', $usuario->nombre) }}" placeholder="ejemplo: Juan"
                                title="Actualiza el nombre del usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                value="{{ old('nombre') }}">
                            @error('nombre')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                        ...
                        <div>
                            ...
                            <select name="empresa" id="empresa" title="Actualiza la empresa a la que pertenece el usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="GÄTSIMED" {{ $usuario->empresa == 'GÄTSIMED' ? 'selected' : '' }}>GÄTSIMED
                                </option>
                        ...
                        <div>
                            <label for="fn" class="block text-base font-medium text-gray-700">Fecha de
                                Nacimiento</label>
                            <input type="date" name="fn" id="fn" value="{{ old('fn', $usuario->fn) }}"
                                title="Actualiza la fecha de nacimiento del usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                value="{{ old('fn') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="gen" id="gen" title="Actualiza el sexo del usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="Femenino" {{ $usuario->gen == 'Femenino' ? 'selected' : '' }}>Femenino
                                </option>
                                <option value="Masculino" {{ $usuario->gen == 'Masculino' ? 'selected' : '' }}>Masculino
                                </option>
                            </select>
                        </div>
                        ...
                        <!-- Selección de Rol -->
                        <div>
                            <label for="rol" class="block text-base font-medium text-gray-700">Selección de
                                Rol</label>
                            <select name="rol" id="rol" title="Actualiza el rol del usuario"
                                ...
                                <option value="Moderador" {{ $usuario->rol == 'Moderador' ? 'selected' : '' }}>Moderador
                                </option>
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email"
                                ...
                                title="Actualiza el correo electrónico del usuario" placeholder="ejemplo@correo.com"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                value="{{ old('email') }}">
                            @error('email')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                        ...
                        <div>
                            ...
                            <input type="text" name="usuario" id="usuario"
                                value="{{ old('usuario', $usuario->usuario) }}" placeholder="Nombre de Usuario"
                                title="Actualiza el nombre de usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                value="{{ old('usuario') }}">
                            @error('usuario')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                        ...
                        <div class="relative col-span-1 md:col-span-2">
                            ...
                            <input type="password" name="pass" id="pass" placeholder="Contraseña"
                                title="Actualiza la contraseña del usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <button type="button" onclick="togglePassword()" title="Ver la contraseña del usuario"
                                class="absolute inset-y-0 right-0 flex items-center px-3 mt-8 text-gray-500">
                                <!-- Icono de Ojo -->
                        ...
                        <div>
                            <label for="estatus" class="block text-base font-medium text-gray-700">Selección de
                                Estatus</label>
                            <select name="estatus" id="estatus" title="Actualizar el estatus del usuario"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option selected>Selecciona una opción...</option>
                                <option value="Activo" {{ $usuario->estatus == 'Activo' ? 'selected' : '' }}>Activo
                                </option>
                                <option value="Inactivo" {{ $usuario->estatus == 'Inactivo' ? 'selected' : '' }}>Inactivo
                                </option>
                            </select>
                        </div>
                        ...
                        <!-- Licencia -->
                        <div>
                            <label for="num_licencia" class="block text-base font-medium text-gray-700">Licencia</label>
                            <input type="num_licencia" name="num_licencia" id="num_licencia"
                                value="{{ old('num_licencia') }}" placeholder="Ejemplo: 987654325"
                                class="w-full px-4 py-2 mt-2 text-base border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                title="Ingresa tu numero de licencia">
                            @error('licencia')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="flex flex-col items-center space-y-4">
                            <label for="foto" class="block text-base font-medium text-gray-700">Seleccionar
                                Imagen</label>
                            <div
                                class="relative flex min-h-[200px] items-center justify-center rounded-md border border-dashed border-[#e0e0e0] mt-4 p-6 text-center">
                                <input type="file" name="foto" id="foto" accept="image/*"
                                    class="absolute inset-0 opacity-0 cursor-pointer"
                                    title="Actualizar la foto del usuario">
                    ...
                    <div class="flex justify-end mt-6">
                        ...
                        <button type="submit" title="Actualizar los datos del usuario"
                            class="px-5 py-3 ml-2 text-white bg-indigo-600 rounded-md shadow-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">Guardar</button>
                    </div>
                </form>
            </div>
        </div>
