...
<div class="container mt-5">
    <div class="">
        <a class="mr-5 bg-green-700 flex items-center gap-2 rounded  bg-primary px-4.5 py-2 font-medium text-white transition hover:bg-green-900"
            href="{{ route('verificaciones.create') }}">Crear nuevo</a>
    </div>
    <div class="mx-4 overflow-hidden rounded-lg shadow-lg md:mx-10">
        ...
        <table class="w-full table-fixed">
            ...
            <tbody class="bg-white">
                @foreach ($verificacion as $vr)
                    <tr>
                        <td class="px-6 py-4 border-b border-gray-200">{{ $vr->marca }}</td>
                        <td class="px-6 py-4 truncate border-b border-gray-200">{{ $vr->placas }}</td>
                        <td class="px-6 py-4 border-b border-gray-200">{{ $vr->NSI }}</td>
                        <td class="px-6 py-4 border-b border-gray-200">
                            <span class="px-2 py-1 text-xs text-white bg-green-500 rounded-full">Active</span>
                        </td>
                        {{-- acciones --}}
                        <td>
                            <a href="{{ route('verificaciones.edit', $vr->id) }}">Editar</a>
                            <a href="{{ route('verificaciones.show', $vr->id) }}">Ver</a>
                            {{-- eliminar --}}
                            <form action="{{ route('verificaciones.destroy', $vr) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button
