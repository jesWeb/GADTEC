...
<div class="container mt-5">
    <div class="">
        <a class="mr-5 bg-green-700 flex items-center gap-2 rounded  bg-primary px-4.5 py-2 font-medium text-white transition hover:bg-green-900"
            href="{{ route('seguros.create') }}">Crear nuevo</a>
    </div>
        ...
        <table class="w-full table-fixed">
            ...
            <tbody class="bg-white">
                @foreach ($seguro as $seg)
                    <tr>
                        <td class="px-6 py-4 border-b border-gray-200">{{ $seg->marca }}</td>
                        <td class="px-6 py-4 truncate border-b border-gray-200">{{ $seg->placas }}</td>
                        <td class="px-6 py-4 border-b border-gray-200">{{ $seg->NSI }}</td>
