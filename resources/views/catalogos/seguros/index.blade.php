    ...
    <div class="px-4 py-6">
        <div class="p-6 bg-white rounded-md shadow-md">
            ...
            <div class="mb-4 text-right">
                <a   href="{{ route('seguros.create') }}"  class="inline-block px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700">Nuevo registro</a>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($seguro as $key => $seg)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 border">{{ $seg->vehiculo }}</td>
                                ...
                                <td class="px-4 py-2 border">
                                    <div class="flex items-center space-x-2">
                                        <!-- Ver -->
                                        <a  href="{{ route('seguros.show', $seg->id) }}" class="inline-flex items-center justify-center w-8 h-8 text-green-600 border border-green-600 rounded hover:bg-green-600 hover:text-white">
                                        ...
                                        <!-- Editar -->
                                        <a  href="{{ route('seguros.edit', $seg->id) }}"  class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 border border-yellow-600 rounded hover:bg-yellow-600 hover:text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                                        ...
                                        <!-- Eliminar -->
                                        <form  action="{{route('seguros.destroy', $seg) }}"  method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
