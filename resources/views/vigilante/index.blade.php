                        ...
                        @foreach ($vigilante as $asignacion)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2 border"
                                    title="Automóvil asignado: {{ $asignacion->automovil->marca ?? 'N/A' }} {{ $asignacion->automovil->modelo ?? '' }}">
                                    {{ $asignacion->automovil->marca ?? 'N/A' }}
                                    {{ $asignacion->automovil->submarca ?? 'N/A' }}
                                    {{ $asignacion->automovil->modelo ?? 'N/A' }}</td>
                                </td>
                                <td class="px-4 py-2 border"
                                    title="Usuario asignado: {{ $asignacion->usuarios->nombre ?? 'N/A' }}">
                                    {{ $asignacion->usuarios->nombre ?? 'N/A' }} {{ $asignacion->usuarios->app ?? 'N/A' }}
                                    {{ $asignacion->usuarios->apm ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border" title="Fecha de asignación">
                                ...
                                <td class="px-4 py-2 border" title="Destino del servicio">{{ $asignacion->lugar ?? 'N/A' }}
                                </td>
                                <td class="px-4 py-2 border" title="Motivo del servicio">{{ $asignacion->motivo ?? 'N/A' }}
                                </td>

                                @if ($asignacion->checkIns->isNotEmpty())
                                    <td class="px-4 py-2 border" title="Kilómetros al salir">
                                        {{ $asignacion->checkIns->first()->km_salida ?? 'N/A' }}</td>
                                    <td class="px-4 py-2 border" title="Nivel de combustible al salir">
                                        {{ $asignacion->checkIns->first()->combustible_salida ?? 'N/A' }}</td>
                                ...
                                @if ($asignacion->checkIns->isNotEmpty())
                                    <td class="px-4 py-2 border" title="Kilómetros al llegar">
                                        {{ $asignacion->checkIns->first()->km_llegada ?? 'N/A' }}</td>
                                    <td class="px-4 py-2 border" title="Nivel de combustible al llegar">
                                        {{ $asignacion->checkIns->first()->combustible_llegada ?? 'N/A' }}</td>
                                ...
                                <td class="px-4 py-2 border">
                                    <div class="flex items-center space-x-2">
                                                ...
                                                @if (auth()->user()->hasRole('Administrador'))
                                                    <a href="{{ route('admin.edit2', $asignacion->id_asignacion) }}"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2" d="M9 5l-7 7 7 7M22 12H2" />
                                                        </svg>
                                        ...
                                        @else
                                            @if (auth()->user()->hasRole('Administrador'))
                                                <a href="{{ route('vigilante.edit', $asignacion->id_asignacion) }}"
                                            ...
                                            @elseif(auth()->user()->hasRole('Moderador'))
                                                <a href="{{ route('moderador.edit', $asignacion->id_asignacion) }}"
                                                    class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 border border-yellow-600 rounded hover:bg-yellow-600 hover:text-white"
                                                    title="Editar información de salida">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
