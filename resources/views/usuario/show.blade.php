            ...
            <div class="flex justify-center mt-10">
                <div class="w-full max-w-3xl p-8 bg-white rounded-xl shadow-lg border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p><strong>Fecha de Registro:</strong>
                                    {{ $asignacion->fecha_asignacion ? date('d/m/Y', strtotime($asignacion->fecha_asignacion)) : 'No registrada' }}
                                </p>
                                <p><strong>Fecha de Salida:</strong>
                                    {{ $asignacion->fecha_salida ? date('d/m/Y', strtotime($asignacion->fecha_salida)) : 'No registrada' }}
                                </p>
                                <p><strong>Motivo:</strong> {{ $asignacion->motivo ?? 'No especificado' }}</p>
                                <p><strong>Lugar:</strong> {{ $asignacion->lugar ?? 'No registrado' }}</p>
                                <p><strong>Especificaciones:</strong> {{ $asignacion->observaciones ?? 'No registradas' }}
                                </p>
                                ...
                                <p><strong>Estatus:</strong> <span
                                        class="font-semibold text-blue-600">{{ $asignacion->estatus ?? 'No especificado' }}</span>
                                </p>
                            </div>
                    ...
                    <div class="mt-6 p-6 bg-gray-50 rounded-lg">
                        ...
                        @php
                            $checkIn = $asignacion->checkIns->first();
                        @endphp

                        <p><strong>Hora de Salida:</strong>
                            {{ $checkIn && $checkIn->hora_salida ? date('h:i A', strtotime($checkIn->hora_salida)) : 'No registrada' }}
                        </p>
                        <p><strong>Hora de Entrada:</strong>
                            {{ $checkIn && $checkIn->hora_llegada ? date('h:i A', strtotime($checkIn->hora_llegada)) : 'No registrada' }}
                        </p>
                        <p><strong>Fecha de Entrada:</strong>
                            {{ $checkIn && $checkIn->fecha_llegada ? date('d/m/Y', strtotime($checkIn->fecha_llegada)) : 'No registrada' }}
                        </p>
                    ...
                    <div class="flex justify-end mt-6">
                        <a href="{{ route('user.dashboard') }}"
                            class="px-6 py-2 text-white transition duration-200 bg-red-600 rounded-md hover:bg-red-700 focus:outline-none">
                            Cerrar
                        </a>
                    </div>
                </div>
